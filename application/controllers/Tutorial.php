...
//$this->load->library(array(''));
        $this->load->model(array('tutorialmodel', 'cv', 'about'));
    ...
    public function list($from = 0) {
        $output = array();
        $config = $this->pagerSettings(base_url('tutorial/list'), $this->tutorialmodel->getCountOfTutorialEntries());
        $this->pagination->initialize($config);
        if (!isset($from)) {
            $output["tutorial"] = $this->tutorialmodel->getFirstTutorialEntry();
        } else {
            $output["tutorial"] = $this->tutorialmodel->getTutorialEntries((int) $from);
        }
        $output['pagelinks'] = $this->pagination->create_links();
        $this->load->view('tutoriallist', $output);
    }

    public function detail($id = 0, $entryid = 0) {
        if ($id > 0) {
            $output = array();
            $output['categoryandcount'] = [];
            $output['dateandcount'] = $this->tutorialmodel->getDateAndCountByTutorialentries();
            $output['tutorialtags'] = [];
            $output['tutorialdetail'] = $this->tutorialmodel->getTutorialEntryById((int) $id);
            $output['recaptcha'] = $this->recaptcha->create_box();
            $output['comments'] = $this->tutorialmodel->getComments((int) $id);
            $output['tutorialentryid'] = $id;
            ...
            if($output['tutorialdetail']['type'] == 'book'){
                if($entryid > 0){
                    $output['tutorialbookdetail']   = $this->tutorialmodel->getTutorialBookEntriesById((int) $id);
                    $output['tutorialEntryId']      = (int)$entryid;
                    $this->load->view('tutorialbook', $output);
                ...
                if ($this->form_validation->run() == FALSE) {
                    $this->load->view('tutorialdetail', $output);
                } else {
                    $reCaptchaIsValid = $this->recaptcha->is_valid();
                    if ($reCaptchaIsValid['success']) {
                        $tutorialEntryId = $this->input->post('tutorialentryid');
                        $name = $this->input->post('fullname');
                        $email = $this->input->post('email');
                        $website = $this->input->post('website');
                        $message = $this->input->post('message');

                        $this->blogmodel->saveComment($tutorialEntryId, $name, $email, $website, $message);
                        redirect('/tutorial/detail/' . (int) $id, 'refresh');
                    } else {
